...
image = record.data.user.image != undefined ? record.data.user.image : 'logo.png';
var globalimage = record.data.user.image != undefined ? record.data.user.image : 'logo.png';
...
socket.on('contactsLists', ({ contacts }) => {
  ...
  if (contacts != null) {
    contacts.forEach((contact) => {
      console.log(contact, "TESTING USERID", contact.user_id)
      const userBox = `
...
socket.on("chat message", function ({ id, message, sender_id, receiver_id, file_upload, createdAt, receiverName, receiverImage, myid }) {
  ...
  if (document.getElementById('s_chat_' + myid) != null) {
    ...
    if (document.getElementById('security-notificationswitch').checked == true) {
      ...
      const receiver_Image = receiverImage ? `assets/images/users/${receiverImage}` : `assets/images/users/logo.png`;
...
socket.on('contactInfo', ({ contacts }) => {
  ...
  `;
    ...
    document.getElementById('headerimg').setAttribute('src', `assets/images/users/logo.png`);
...
socket.on("currentUser", function ({ userInfo }) {
  ...
  Array.from(preview).forEach((element, index) => {
    var profile_img = userInfo.image ? userInfo.image : "logo.png";
...
document.querySelector("#btnSave").addEventListener("click", (e) => {
  ...
  if (name == '') {
    toastr.error(`A Name is Required`, "Error");
  }
  else {
    var userName = document.querySelectorAll(".user_name");
    Array.from(userName).forEach((element, index) => {
